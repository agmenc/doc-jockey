...
case class ComputerIsNow(expected: Boolean) extends JustACommand {
  import Implicits._
...
class AssertComputerIsNowSpec extends WordSpec {
  // Thingys
  "Executing a Before gives you an After" in {
    val aCmd = ComputerIsNow(true)
    assert(Before(aCmd).execute === After(aCmd, List(Pass("on"))))
  }

  "Befores can render themselves" in {
    assert(Before(ComputerIsNow(true)).render === <table><tr><td>Computer is</td><td>on</td></tr></table>)
  }

  "Afters can render themselves" in {
    assert(After(ComputerIsNow(true), List(Fail("off", "on"))).render === <table><tr><td>Computer is</td><td><span class="failText">off</span>on</td></tr></table>)
  }

  // Commands
  "Executing a command gives you a list of results" in {
    assert(ComputerIsNow(true).execute === List(Pass("on")))
  }

  "Failed commands have failure results" in {
    assert(ComputerIsNow(false).execute === List(Fail("off", "on")))
  }

  "A command can render code for itself" in {
    assert(ComputerIsNow(false).inCode === "ComputerIsNow(false)")
  }
}